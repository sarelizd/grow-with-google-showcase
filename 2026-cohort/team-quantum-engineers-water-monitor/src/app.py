...
/* ---------- Table (st.table) ----------
   Used for the two full-detail tables (Water Safety Status by Region,
   Municipal Water Testing Results) instead of st.dataframe, so every
   column and row renders at full size with wrapped text, never an
   ellipsis cutoff or a horizontal scrollbar, and never a blank filler
   row when a filter narrows the result set to fewer rows than a fixed
   grid height would otherwise pad out. */
[data-testid="stTable"]{
    background:rgba(255,255,255,.10);
    border:1px solid rgba(255,255,255,.16);
    border-radius:16px;
...
with tab_map:
    ...
    if total_samples == 0:
        ...
        # st.table (not st.dataframe) renders every column and row at full
        # size with wrapped text instead of ellipsis truncation, and with
        # no fixed-height scroll container — so nothing is ever cut off,
        # no horizontal scrollbar is needed, and a filtered result with
        # only a few rows never gets padded out with blank rows.
        st.table(map_table.style.apply(highlight_risk, axis=1).hide(axis="index"))
...
with tab_quality:
    ...
    # st.table, same reasoning as the Water Safety Status by Region table
    # above: every column and row renders in full, with text wrapping
    # instead of truncation, no horizontal scrollbar, and no blank filler
    # rows when a filter narrows the results down.
    st.table(quality_table.style.apply(highlight_risk, axis=1).hide(axis="index"))

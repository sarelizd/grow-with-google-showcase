...
with tab_map:
    ...
    if total_samples == 0:
        ...
        st.dataframe(
            ...
            hide_index=True,
            height=340,
            # Explicit widths (all "large" for text columns) so Region and
            # Water Source Type values render in full, without ellipsis
            # cutoffs, the same way the Municipal Water Testing Results
            # table below is configured.
            column_config={
                "Year": st.column_config.NumberColumn(width="small", format="%d"),
                "Region": st.column_config.TextColumn(width="large"),
                "Water Source Type": st.column_config.TextColumn(width="large"),
...
with tab_quality:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Bacteria Count (CFU/mL)": st.column_config.NumberColumn(width="medium"),
            "Contaminant Level (ppm)": st.column_config.NumberColumn(width="medium"),
            "Dissolved Oxygen (mg/L)": st.column_config.NumberColumn(width="medium"),

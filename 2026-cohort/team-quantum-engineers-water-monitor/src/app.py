...
def load_data(path: str) -> pd.DataFrame:
    ...
    # For every other row, this names whichever disease (Cholera, Typhoid,
    # or Diarrheal) has the highest reported incidence, in a short 4-7 word
    # label rather than a full sentence. Cholera/Typhoid severity bands are
    # WHO-referenced; Diarrheal has no official absolute WHO threshold, so
    # it's flagged only when it's elevated relative to this dataset's own
    # distribution (above the 75th percentile).
    ...
    def disease_indicator_note(row):
        ...
        candidates = []  # (incidence_value, short_label)

        cholera = row.get("Cholera Cases per 100,000 people")
        if pd.notna(cholera):
            if cholera >= 10:
                candidates.append((cholera, "High incidence risk for Cholera"))
            elif cholera >= 1:
                candidates.append((cholera, "Moderate incidence risk for Cholera"))

        typhoid = row.get("Typhoid Cases per 100,000 people")
        if pd.notna(typhoid):
            if typhoid >= 100:
                candidates.append((typhoid, "High incidence risk for Typhoid"))
            elif typhoid >= 10:
                candidates.append((typhoid, "Moderate incidence risk for Typhoid"))

        diarrheal = row.get("Diarrheal Cases per 100,000 people")
        if pd.notna(diarrheal) and pd.notna(diarrheal_p75) and diarrheal >= diarrheal_p75:
            candidates.append((diarrheal, "Elevated diarrheal disease rate reported"))
...
with tab_map:
    ...
    if total_samples == 0:
        ...
        st.dataframe(
            ...
            hide_index=True,
            height=340,
            # "medium" (not "large") for Region and Water Source Type so
            # all 6 columns fit within the table's width — with both set
            # to "large", the row total ran wider than the visible area,
            # which pushed Turbidity (NTU) off the right edge and made it
            # look cut off / require the horizontal scrollbar to reach.
            # Year is narrowed to a fixed pixel width (its values are only
            # 4 digits, so "small" left unused space) to free up room so
            # the Turbidity (NTU) header/values are no longer clipped.
            column_config={
                "Year": st.column_config.NumberColumn(width=70, format="%d"),
                "Region": st.column_config.TextColumn(width="medium"),
                "Water Source Type": st.column_config.TextColumn(width="medium"),
                "Risk Level": st.column_config.TextColumn(width="medium"),
                "pH Level": st.column_config.NumberColumn(width="small"),
                "Turbidity (NTU)": st.column_config.NumberColumn(width=130),
            },
        )
...
with tab_quality:
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "pH Level": st.column_config.NumberColumn(width="small"),
            # Turbidity's header ("Turbidity (NTU)") is longer than it
            # looks, so "small" was clipping the header text itself even
            # though the values are short — back to "medium" fixes that.
            # The other three still only need "small" since their values
            # are short and their headers are narrower relative to it.
            "Turbidity (NTU)": st.column_config.NumberColumn(width="medium"),
            "Bacteria Count (CFU/mL)": st.column_config.NumberColumn(width="small"),
            "Contaminant Level (ppm)": st.column_config.NumberColumn(width="small"),
            "Dissolved Oxygen (mg/L)": st.column_config.NumberColumn(width="small"),
            "Water Treatment Method": st.column_config.TextColumn(width="medium"),
            "Risk Level": st.column_config.TextColumn(width="medium"),
            "Risk Reason": st.column_config.TextColumn(width="large"),
            "Disease Indicator / Risk": st.column_config.TextColumn(width="large"),
        },
    )
    st.caption("🟢 Safe   →   🟠 Moderate Risk   →   🔴 High Risk — matching the colors on the map above.")
    st.caption(
        "Disease Indicator / Risk shows the original field-recorded note for Abuja and Benue South. "
        "For all other locations, it names whichever disease (Cholera, Typhoid, or Diarrheal) has the "
        "highest reported incidence, using WHO-referenced severity bands where available."
    )

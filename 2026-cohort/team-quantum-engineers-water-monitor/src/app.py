...
def load_data(path: str) -> pd.DataFrame:
    ...
    def field_reported_reasons(row):
        ...
        if weight == 0:
            return []
        label = f"Field-reported contamination: {str(note).split('(')[0].strip()} ({note})"
        return [label] * weight
...
with st.sidebar:
    ...
    st.caption(
        ...
        "exactly 1 does, and Safe otherwise. For the 9 Abuja/Benue South field-survey "
        "rows that have no numeric Bacteria Count or Contaminant Level (ppm) reading, "
        "the qualitative field note (e.g. \"Extreme\", \"Very High\", \"High\") is used "
        "in its place, so those sites are still screened on contamination severity "
        "instead of being judged on pH/Turbidity alone."
    )
...
with tab_quality:
    ...
    st.caption(
        ...
        "highest reported incidence relative to this dataset's own distribution."
    )
    st.caption(
        "⚠️ Risk Level and Disease Indicator / Risk are two different measurements and can legitimately "
        "disagree: Risk Level grades this specific water sample against safety thresholds, while Disease "
        "Indicator / Risk reflects the surrounding region's reported illness rates — a site can test clean "
        "today while its region still carries a disease burden from other causes (storage, sanitation, a "
        "past outbreak)."
    )

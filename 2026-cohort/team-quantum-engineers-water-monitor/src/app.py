...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption(
        "**Safety thresholds:** Only pH (6.5–8.5) and Turbidity "
        f"(≤ {TURBIDITY_MAX} NTU) are official WHO drinking-water guideline values. "
        f"Bacteria (≤ {int(BACTERIA_MAX)} CFU/mL) and Contaminant Level (≤ {CONTAMINANT_MAX} ppm) "
        "are **not** WHO figures — they are dataset-calibrated cutoffs, since the source data "
        "doesn't identify which pathogen or contaminant was tested (WHO's real standard for "
        "bacteria like E. coli is zero tolerance). High Risk = 2+ thresholds breached, "
...
with tab_quality:
    ...
    st.info(
        "ℹ️ **Two different signals, side by side.** *Risk Level* grades only the water "
        "sample in that row (pH, turbidity, bacteria, contaminant). *Disease Indicator / "
        "Risk* is normally the surrounding region's Cholera/Typhoid/Diarrheal case rate — "
        "a public-health signal, not a water-test result — **except** for the 9 Abuja/Benue "
        "South rows, where it instead shows the original field-recorded contamination alert "
        "for that specific sample (no regional case-rate data was collected for those sites). "
        "A row can show **Safe** water and a **High** disease indicator at the same time: the "
        "water sample passed its checks, but the region still carries a high case rate from "
        "other causes (storage, distribution, sanitation, a past outbreak). This is expected, "
        "not an error."
    )
    ...
    with s4:
        st.markdown(
            ...
            <div class="glass-card action-card">
                ...
                <div class="action-title">Watch the disease indicator too</div>
                <div class="action-copy">
                    A Safe water sample doesn't rule out a regional disease signal. If Disease
                    Indicator / Risk is Moderate or High, use safe storage and handling
                    (covered containers, clean hands/utensils) and stay alert to symptoms locally.
                </div>
            </div>
            """).strip(),
            unsafe_allow_html=True,

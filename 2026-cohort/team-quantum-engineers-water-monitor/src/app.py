...
with tab_quality:
    ...
    st.dataframe(
        ...
        # values underneath are short. "Risk Reason" and especially
        # "Disease Indicator / Risk" can also hold much longer text
        # (up to ~65 and ~113 characters respectively) than "large"
        # comfortably shows. Widening every column to its own measured
        # max means no header or cell truncates; the tradeoff is that
        # a 12-column table with two descriptive-text columns is wide
        # enough to need horizontal scrolling to see every column at
        # once — that's expected for a table this rich, and is not the
        # same thing as text being cut off.
        column_config={
            ...
            "pH Level": st.column_config.NumberColumn(width=100),
            "Turbidity (NTU)": st.column_config.NumberColumn(width=145),
            "Bacteria Count (CFU/mL)": st.column_config.NumberColumn(width=210),
            "Contaminant Level (ppm)": st.column_config.NumberColumn(width=210),
            ...
            # Narrowed from 760 back to 720 — that's the width computed
            # directly from this column's longest real entry (113 chars)
            # plus normal padding, with the extra "can't render it myself
            # to verify" safety margin removed now that 760 is confirmed
            # to show every entry with no truncation. 720 is the floor
            # for zero truncation on single-line text; going narrower
            # than this risks clipping that longest entry.
            "Disease Indicator / Risk": st.column_config.TextColumn(width=720),
        },
    )

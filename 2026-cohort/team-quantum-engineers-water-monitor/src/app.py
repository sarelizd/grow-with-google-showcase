...
# Used only to draw a stroke outline on the map so filtered/jittered
# points are visibly anchored to Nigeria; not used for any calculation.
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption(
        f"**Safety thresholds:** Only pH ({PH_MIN}–{PH_MAX}) and Turbidity "
...
with tab_map:
    ...
    if total_samples == 0:
        ...
        # Outline-only layer (no fill) so it frames the country without
        # covering the road basemap or the risk-level points.
        border_layer = pdk.Layer(
            "GeoJsonLayer",
            data=NIGERIA_BORDER_GEOJSON,
            stroked=True,
            filled=False,
            get_line_color=[15, 79, 191],  # matches the --bg-cobalt accent
        ...
        st.pydeck_chart(
            pdk.Deck(
                layers=[border_layer, layer],
                initial_view_state=view_state,
                tooltip=tooltip,
                map_provider="carto",
                map_style="satellite",  # satellite imagery; border layer above frames Nigeria since satellite has no labeled borders
            )
        )
    ...
    st.markdown(
        ...
        <div class="action-grid">
            ...
            <div class="glass-card action-card">
                ...
                <div class="action-title">Check the Disease Indicator / Risk too</div>
                <div class="action-copy">
                    A water sample that's <b>Safe</b> doesn't rule out a regional disease signal. Whenever
                    <b>Disease Indicator / Risk</b> names any concern, such as a Cholera/Typhoid/Diarrheal
                    incidence note, or a field alert like "Pathogen Alert" or "Severe Pathogen
                    Alert," use safe storage and handling (covered containers, clean
                    hands/utensils) and stay alert to symptoms locally. Only "No elevated
